...
from services.rvc.lib.predictors.RMVPE import RMVPE0Predictor
from services.rvc.lib.predictors.FCPE import FCPEF0Predictor
...
class Pipeline:
    ...
    def __init__(self, tgt_sr, config):
        ...
        self.model_rmvpe = RMVPE0Predictor(
            os.path.join("services", "rvc", "models", "predictors", "rmvpe.pt"),
        ...
        for method in methods:
            ...
            if method == "crepe":
                ...
                self.model_fcpe = FCPEF0Predictor(
                    os.path.join("services", "rvc", "models", "predictors", "fcpe.pt"),
                    f0_min=int(f0_min),
                    f0_max=int(f0_max),
                    dtype=torch.float32,
                    device=self.device,
    ...
    def _retrieve_speaker_embeddings(self, feats, index, big_npy, index_rate):
        ...
        # print(f"ix : {ix}")
        # print("We got here")
        score = np.ones((npy.shape[0], 8), dtype=np.float32)  # Fake "distances" (all 1s)
        ix = np.zeros((npy.shape[0], 8), dtype=np.int64)      # Fake "indices" (all 0s)
